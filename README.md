...
This repository contains the submission for the Post Doctoral Research Associate (91336) technical task at the Liverpool School of Tropical Medicine, MalariaGEN Vector Observatory.
...
bash
conda create -n pdra_test python=3.11 -y
conda activate pdra_test


### 2. Install core packages via conda
bash
conda install -c conda-forge malariagen_data pandas numpy -y
...
| **Relevant output** (DataFrame) | `test_full_pipeline_with_mocked_ezstructure` | Asserts shape, column names (`Ancestry_1`, `Ancestry_2`), sample ID index, and row sums ≈ 1.0 |
...
* **Output:** Returns a `pandas.DataFrame` indexed by sample ID (read from the generated `.fam` file), with columns `Ancestry_1` through `Ancestry_K`. Each row sums to 1.0.

## References

1. Raj, A., Stephens, M. & Pritchard, J.K. (2014). **fastSTRUCTURE: Variational Inference of Population Structure in Large SNP Data Sets.** *Genetics*, 197(2), 573–589.
...
3. MalariaGEN Vector Observatory. **malariagen_data Python package.** https://github.com/malariagen/malariagen-data-python
\end{lstlisting}# MDD
