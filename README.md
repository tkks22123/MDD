...
- **Testing strategy & Data Access:** Due to the standard approval timelines required to obtain Google Cloud data access credentials for the MalariaGEN datasets, the test suite is built using `unittest.mock`. This isolates the class from both the Google Cloud backend and the local `ezstructure` binary. This guarantees that the full admixture workflow is rigorously verified without relying on external network or authentication. It also ensures the tests are fully portable and can be run instantly by reviewers. Once authenticated access is available in a production environment, the Mixin will seamlessly retrieve data via the native `malariagen_data` API.
